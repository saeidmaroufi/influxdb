...
func (i *Partition) compactToLevel(files []*IndexFile, level int, interrupt <-chan struct{}) {
	...
	log, logEnd := logger.NewOperation(i.logger, "TSI level compaction", "tsi1.compact_to_level", zap.Int("tsi1.level", level))
	defer logEnd()
...
func (i *Partition) compactLogFile(logFile *LogFile) {
	...
	// Build a logger for this compaction.
	log, logEnd := logger.NewOperation(i.logger, "TSI log compaction", "tsi1.compact_log_file", zap.Int("tsi1.log_file_id", id))
	defer logEnd()
